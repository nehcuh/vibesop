  ...
  module SuperpowersInstaller
    ...
    def self.install_superpowers_for_platform(platform)
      ...
      unless Dir.exist?(source_skills_dir)
        ...
        puts "   Repository: #{SUPERPOWERS_REPO_URL}"
        puts "   Target: #{shared_dir}"

        _stdout, stderr, status = Open3.capture3(
          "git", "clone", "--depth", "1", SUPERPOWERS_REPO_URL, shared_dir
        )

        unless status.success?
          puts "   ❌ Failed to clone Superpowers repository"
      ...
      unless symlink_config
        ...
        puts "   1. Clone: git clone #{SUPERPOWERS_REPO_URL} #{shared_dir}"
