...
class ConfigDrivenRenderersTester
  ...
  def initialize(repo_root)
    ...
    @tiers_doc = YAML.safe_load(
      File.read(File.join(repo_root, 'core/models/tiers.yaml')), aliases: true
    )
    providers_path = File.join(repo_root, 'core/models/providers.yaml')
    @providers = if File.exist?(providers_path)
                   YAML.safe_load(File.read(providers_path),
                                  aliases: true)
                 else
                   {}
                 end
...
class TestConfigDrivenRenderers < Minitest::Test
  ...
  def test_render_opencode_creates_expected_structure
    ...
    assert File.exist?(File.join(vibe_dir, 'safety.md'))
    assert File.exist?(File.join(vibe_dir, 'task-routing.md')),
           'Should have task-routing.md (aligned with Claude Code)'
    assert File.exist?(File.join(vibe_dir, 'test-standards.md')),
           'Should have test-standards.md (aligned with Claude Code)'
  end
  ...
  def test_generate_native_config_raises_when_builder_missing
    ...
    assert_raises(ArgumentError) do
      @renderer.send(:generate_native_config, @build_root, @base_manifest, config,
                     'global')
    end
  end

  def test_generate_native_config_raises_on_unsupported_type
    config = { 'filename' => 'x.xml', 'builder' => 'build_claude_settings',
               'type' => 'xml' }
    assert_raises(ArgumentError) do
      @renderer.send(:generate_native_config, @build_root, @base_manifest, config,
                     'global')
