...
class TestNativeConfigGeneration < Minitest::Test
  ...
  def test_claude_code_generates_settings_json
    ...
    assert content['permissions']['ask'].is_a?(Array), "permissions.ask should be an array"
    assert content['permissions']['deny'].is_a?(Array), "permissions.deny should be an array"
  end
