...
class TestBuildCommand < Minitest::Test
  ...
  def test_build_opencode_creates_all_expected_files
    ...
    assert File.exist?(File.join(output, '.vibe/opencode/task-routing.md')),
           'Should have task-routing.md (aligned with Claude Code)'
    assert File.exist?(File.join(output, '.vibe/opencode/test-standards.md')),
  ...
  def test_build_with_overlay_applies_overlay
    ...
    assert_equal 'example-regulated-project', manifest['overlay']['name'],
                 'Overlay name should be recorded'
  end
