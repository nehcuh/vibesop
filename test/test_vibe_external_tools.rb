...
class TestVibeExternalTools < Minitest::Test
                                                ...
                                                'PreToolUse' => [
                                                  {
                                                    'matcher' => 'Bash',
                                                    'hooks' => [{ 'type' => 'command',
                                                                  'command' => 'rtk proxy -- something' }]
                                                  }
                                                ]
                                              }
                                            }))

    File.stub :expand_path, lambda { |p, *|
                              p == '~/.claude/settings.json' ? settings_path : File.expand_path(p)
                            } do
      assert rtk_hook_configured?
    end
  end
  ...
  def test_rtk_hook_configured_false_when_settings_file_missing
    ...
    File.stub :expand_path, lambda { |p, *|
                              p == '~/.claude/settings.json' ? missing_path : File.expand_path(p)
                            } do
      refute rtk_hook_configured?
    end
  end
  ...
  def test_pending_integrations_returns_keys_not_ready
    ...
    stub(:integration_status, fake_status) do
  ...
  def test_verify_rtk_non_claude_code_platform_does_not_require_hook
    ...
    stub(:detect_rtk, :not_installed) do
      stub(:rtk_hook_configured?, false) do
