...
### P020: AI 路由功能反复修改的根本原因分析 (2026-03-30)
- **场景**: AI 路由功能在审查过程中发现多个问题，需要反复修复
  - 自动路由不触发
  - AI 匹配一切（包括无关请求）
  - 限制用户选择
  - Claude Code 调用外部 API
- **根因分析**:
  1. **需求分析不足** — 没有在实现前定义所有运行环境场景
  2. **过度承诺** — README 声称的功能与实际实现不符
  3. **边界条件滞后** — "无匹配"场景作为事后补充
  4. **错误的心智模型** — AI 作为决策者而非建议者
  5. **环境盲点** — 没有检查 Claude Code 已有内置 AI 能力
  6. **补丁式迭代** — 3+ 次修改同一区域而非重新设计
- **预防措施**（Implementation Checklist）:
  1. **场景矩阵先行**: 列出所有运行环境及其行为
  2. **边界条件优先**: 先定义无匹配/失败场景
  3. **哲学声明**: "AI 建议，用户决定" vs "AI 决定"
  4. **环境检测**: 检查环境是否已解决问题
  5. **可观测性**: 添加 `--stats` 命令
  6. **3 次补丁规则**: 停止补丁，重新设计
- **创建文档**: `docs/architecture/ai-routing-retrospective.md`, `docs/implementation-checklist.md`
- **使用次数**: 1（但适用于所有 AI/外部服务功能）
- **Commit**: 待提交
